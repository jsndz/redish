...
func TestDispatch(t *testing.T) {
	...
	t.Run("Dispatch PING", func(t *testing.T) {
		res, err := Dispatch(c, []interface{}{"PING"}, s, nil, nil, nil)
		if err != nil {
	...
	t.Run("Dispatch MULTI and EXEC", func(t *testing.T) {
		res, _ := Dispatch(c, []interface{}{"MULTI"}, s, nil, nil, nil)
		if string(res) != "+OK\r\n" {
		...
		res, _ = Dispatch(c, []interface{}{"SET", "k1", "v1"}, s, nil, nil, nil)
		if string(res) != "+QUEUED\r\n" {
			t.Errorf("Expected +QUEUED, got %q", string(res))
		}

		res, _ = Dispatch(c, []interface{}{"GET", "k1"}, s, nil, nil, nil)
		if string(res) != "+QUEUED\r\n" {
			t.Errorf("Expected +QUEUED, got %q", string(res))
		}

		res, _ = Dispatch(c, []interface{}{"EXEC"}, s, nil, nil, nil)
		expected := "*2\r\n+OK\r\n$2\r\nv1\r\n"
		if string(res) != expected {
			t.Errorf("Expected %q, got %q", expected, string(res))
		}
	...
	t.Run("Dispatch DISCARD WITH MULTI", func(t *testing.T) {
		...
		res, _ = Dispatch(c, []interface{}{"DISCARD"}, s, nil, nil, nil)
		expected := "+OK\r\n"
		if string(res) != expected {
			t.Errorf("Expected %q, got %q", expected, string(res))
		}
	...
	t.Run("Dispatch DISCARD WITHOUT MULTI", func(t *testing.T) {
		...
		_, err := Dispatch(c, []interface{}{"DISCARD"}, s, nil, nil, nil)
		if err == nil {
	...
	t.Run("EXEC does not rollback on command error", func(t *testing.T) {
		...
		Dispatch(c, []interface{}{"SET", "foo", "bar"}, s, nil, nil, nil)

		Dispatch(c, []interface{}{"GET"}, s, nil, nil, nil)
		...
		res, err := Dispatch(c, []interface{}{"GET", "foo"}, s, nil, nil, nil)
		if err != nil {
